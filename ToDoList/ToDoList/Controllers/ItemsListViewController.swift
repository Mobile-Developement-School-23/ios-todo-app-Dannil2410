...
class ItemsListViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(saveItems), name: UIApplication.didEnterBackgroundNotification, object: nil)
...
extension ItemsListViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row != rowsCount {
            ...
            cell.configureCell(rowInSection: 1+rowsCount, currentRow: indexPath.row, hasDeadLine: item.deadLine != nil ? true : false, lastCell: false)
...
extension ItemsListViewController: TableViewRowAppendable {
    func updateTableView(showOrHideAncestor: ShowAction) {
        items = showOrHideAncestor == .hide ? fileCache.items.sorted { $0.startTime > $1.startTime} : fileCache.items.filter({$0.isDone == false}).sorted { $0.startTime > $1.startTime}
